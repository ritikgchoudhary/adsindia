<div class="row">
    ...
    <div class="col-md-{{ (isset($data->width) && $data->width) ? $data->width : '12' }}">
            <div class="form-group">
                <label class="form-label form--label">{{ __($data->name) }} @if(isset($data->instruction) && $data->instruction) <span data-bs-toggle="tooltip" title="{{ __($data->instruction) }}"><i class="fas fa-info-circle"></i></span> @endif @if($data->is_required == 'required' && ($data->type == 'checkbox' || $data->type == 'radio')) <span class="text--danger">*</span> @endif </label>
                @if($data->type == 'text')
                    <input type="text"
