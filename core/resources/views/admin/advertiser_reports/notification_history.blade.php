...
@section('panel')
                            ...
                            <tbody>
                                @forelse($logs as $log)
                                    <tr>
                                        <td>
                                            @if ($log->advertiser)
                                                <span class="fw-bold">{{ $log?->advertiser?->fullname }}</span>
                                                <br>
                                                <span class="small">
                                                    <a href="{{ route('admin.advertisers.detail', $log->advertiser_id) }}"><span>@</span>{{ $log?->advertiser?->username }}</a>
                                                </span>
                                            @else
                                        ...
                                        <td>
                                            <button class="btn btn-sm btn-outline--primary notifyDetail" data-type="{{ $log->notification_type }}" @if ($log->notification_type == 'email') data-message="{{ route('admin.advertiser.report.email.details', $log->id) }}" @else data-message="{{ $log->message }}" @if ($log->image) data-image="{{ asset(getFilePath('push') . '/' . $log->image) }}" @endif @endif data-sent_to="{{ $log->sent_to }}"><i class="las la-desktop"></i> @lang('Detail')</button>
                                        </td>
                                    </tr>
                                    @empty
                                        <tr>
    ...
    @push('breadcrumb-plugins')
        @if (isset($advertiser))
            <a href="{{ route('admin.advertisers.notification.single', isset($advertiser) ? $advertiser->id : 0) }}" class="btn btn-outline--primary btn-sm"><i class="las la-paper-plane"></i> @lang('Send Notification')</a>
        @else
