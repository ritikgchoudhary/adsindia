            ...
            <div class="row  mb-4 justify-content-center">
                <div class="col-sm-7">
                    <div class="row gy-4 justify-content-center">
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fas fa-list" :title="$viaText . ' should be sent'" value="{{ isset($sessionData['total_advertiser']) ? $sessionData['total_advertiser'] : '' }}"
                        ...
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fa-solid fa-paper-plane" :title="$viaText . ' has yet to be sent'" :viewMoreIcon=false
                                value="{{ (isset($sessionData['total_advertiser']) ? $sessionData['total_advertiser'] : 0) - (isset($sessionData['total_sent']) ? $sessionData['total_sent'] : 0) }}" bg="warning" />
                        </div>
                        <div class="col-sm-6">
                                ...
                                <div class="card-body p-5 text-center">
                                    <div class="coaling-loader flex-column d-flex justify-content-center">
                                        ...
                                        <p class="text--primary">
                                            @lang(' ' . (isset($sessionData['total_sent']) ? $sessionData['total_sent'] : 0) . ' out of ' . (isset($sessionData['total_advertiser']) ? $sessionData['total_advertiser'] : 0) . ' ' . $viaName . ' were successfully transmitted')
                                        </p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="being_sent_to" required data-minimum-results-for-search="1">
                                        @foreach ($notifyToAdvertiser as $key => $toAdvertiser)
                                            <option value="{{ $key }}" @selected(old('being_sent_to', (isset($sessionData['being_sent_to']) ? $sessionData['being_sent_to'] : '')) == $key)>{{ __($toAdvertiser) }}</option>
                                        @endforeach
                                    </select>
                                    <small class="text--info d-none advertiserCountText"> <i class="las la-info-circle"></i> <strong
                                            class="advertiserCount">0</strong> @lang('active advertisers found to send the notification')</small>
                                </div>
        ...
        (function($) {
            ...
            $('select[name=being_sent_to]').on('change', function(e) {
                ...
                if (methodName == 'SELECTEDADVERTISERS') {
                    $('.input-append').html(`
                    <div class="form-group" id="advertiser_list_wrapper">
                        <label class="required">@lang('Select Advertiser')</label>
                        <select name="advertiser[]"  class="form-control" id="advertiser_list" required multiple >
                ...
                if (methodName == 'TOPDEPOSITEDADVERTISERS') {
                    $('.input-append').html(`
                    <div class="form-group">
                        <label class="required">@lang('Number Of Top Deposited Advertiser')</label>
                ...
                if (methodName == 'NOTLOGINADVERTISERS') {
                    $('.input-append').html(`
                    <div class="form-group">
            ...
            function fetchAdvertiserList() {
                ...
                $('.row #advertiser_list').select2({
                    ajax: {
                        url: "{{ route('admin.advertisers.list') }}",
                        type: "get",
                        dataType: 'json',
                        delay: 1000,
                    ...
                    dropdownParent: $('.input-append #advertiser_list_wrapper')
                });
            ...
            function getAdvertiserCount(methodName) {
                ...
                if (methodNameUpper == 'SELECTEDADVERTISERS' || methodNameUpper == 'ALLADVERTISERS' || methodNameUpper == 'TOPDEPOSITEDADVERTISERS' ||
                    ...
                    $('.advertiserCount').text(0);
                    $('.advertiserCountText').addClass('d-none');
                    return;
                }
                var route = "{{ route('admin.advertisers.segment.count', ':methodName') }}"
                route = route.replace(':methodName', methodName)
                $.get(route, function(response) {
                    $('.advertiserCount').text(response);
                    $('.advertiserCountText').removeClass('d-none');
                });
            }
