            ...
            <div class="card custom--card">
                <div class="card-header card-header-bg d-flex flex-wrap justify-content-between align-items-center">
                    <h5 class="text-white mb-0 d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <span class="ms-1">[@lang('Ticket')#{{ $myTicket->ticket }}] {{ $myTicket->subject }}</span>
                    </h5>
                    @if ($myTicket->status != Status::TICKET_CLOSE && $myTicket->advertiser)
                        <button class="btn btn--danger close-button btn--sm confirmationBtn" type="button" data-question="@lang('Are you sure to close this ticket?')" data-action="{{ route('advertiser.ticket.close', $myTicket->id) }}"><i class="fas fa-lg fa-times"></i>
                        </button>
                    @endif
                </div>
                <div class="card-body">
                    <form method="post" class="disableSubmission" action="{{ route('advertiser.ticket.reply', $myTicket->id) }}" enctype="multipart/form-data">
                        @csrf
                                    ...
                                    @if ($message->attachments->count() > 0)
                                        <div class="mt-2">
                                            @foreach ($message->attachments as $k => $image)
                                                <a href="{{ route('advertiser.ticket.download', encrypt($image->id)) }}" class="me-3 text--base"><i class="fa-regular fa-file"></i> @lang('Attachment') {{ ++$k }} </a>
                                            @endforeach
                                        </div>
                                    @endif
                                </div>
