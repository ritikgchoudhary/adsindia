...
class ForgotPasswordController extends Controller
{
    ...
    public function sendResetCodeEmail(Request $request)
    {
        ...
        $advertiser = Advertiser::where($fieldType, $request->value)->first();
        ...
        PasswordReset::where('email', $advertiser->email)->delete();
        $code = verificationCode(6);
        $password = new PasswordReset();
        $password->email = $advertiser->email;
        $password->token = $code;
        $password->created_at = \Carbon\Carbon::now();
        $password->save();

        $advertiserIpInfo = getIpInfo();
        $advertiserBrowserInfo = osBrowser();
        notify($advertiser, 'PASS_RESET_CODE', [
            'code' => $code,
            'operating_system' => isset($advertiserBrowserInfo['os_platform']) ? $advertiserBrowserInfo['os_platform'] : '',
            'browser' => isset($advertiserBrowserInfo['browser']) ? $advertiserBrowserInfo['browser'] : '',
