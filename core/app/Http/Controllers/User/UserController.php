...
class UserController extends Controller {
    public function home() {
        ...
        $widget['total_earning']  = Conversion::where('user_id', $user->id)->where('is_paid', Status::PAID)->sum('user_payout');
        $widget['ads_income']     = Transaction::where('user_id', $user->id)->where('remark', 'ad_view_reward')->where('trx_type', '+')->sum('amount');
        ...
        $campaigns                = Campaign::running()->whereHas('category', function ($query) {
            $query->active();
        })->take(8)->latest()->get();
        return view('Template::user.dashboard', compact('pageTitle', 'widget', 'transactions', 'campaigns'));
    }

    public function conversionLog(Request $request) {
        $pageTitle   = 'Conversion Log';
        $conversions = Conversion::where('user_id', auth()->id())->with('campaign')->searchable(['campaign:title'])->latest()->paginate(getPaginate());
        return view('Template::user.conversions_log', compact('pageTitle', 'conversions'));
    }
