...
function paginateLinks($data, $view = null, $query = []) {
    return $data->appends(array_merge(request()->all(), $query))->links($view);
}
...
function showBreadcrumb() {
    ...
    return !in_array(Route::currentRouteName(), $excludedRoutes);
}

function isDashboardRoute() {
    return request()->routeIs('user.*') || request()->routeIs('advertiser.*');
}

function isTicketRouteForAuthenticated() {
    $isLoggedIn    = auth()->check() || auth()->guard('advertiser')->check();
    $isTicketRoute = request()->routeIs('ticket.*');

    return $isLoggedIn && $isTicketRoute;
}
...
function advertiserId() {
    return auth()->guard('advertiser')->id();
}
function advertiser() {
