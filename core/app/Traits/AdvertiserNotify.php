...
trait AdvertiserNotify {
    public static function notifyToAdvertiser() {
        return [
            ...
            'kycUnverified'                => 'Kyc Unverified Advertisers',
            'kycVerified'                  => 'Kyc Verified Advertisers',
            'kycPending'                   => 'Kyc Pending Advertisers',
            'withBalance'                  => 'With Balance Advertisers',
            'emptyBalanceAdvertisers'      => 'Empty Balance Advertisers',
            'twoFaDisableAdvertisers'      => '2FA Disable Advertiser',
            'twoFaEnableAdvertisers'       => '2FA Enable Advertiser',
            ...
            'pendingTicketAdvertiser'      => 'Pending Ticket Advertisers',
            'answerTicketAdvertiser'       => 'Answer Ticket Advertisers',
            'closedTicketAdvertiser'       => 'Closed Ticket Advertisers',
            'notLoginAdvertisers'          => 'Last Few Days Not Login Advertisers',
            'hasCampaignAdvertisers'       => 'Campaign Advertisers',
            'notCampaignAdvertisers'       => 'Not Campaign Advertisers',
        ];
    }

    public function scopeSelectedAdvertisers($query) {
        return $query->whereIn('id', request()->advertiser ?? []);
    }
    ...
    public function scopeTopDepositedAdvertisers($query) {
        return $query->whereHas('deposits', function ($deposit) {
            ...
            $q->successful();
        }], 'amount')->orderBy('deposits_sum_amount', 'desc')->take(request()->number_of_top_deposited_advertiser ?? 10);
    }
    ...
    public function scopeHasCampaignAdvertisers($query) {
        return $query->whereHas('campaigns', function ($q) {
            $q->where('status', Status::CAMPAIGN_APPROVED);
        });
    }
    public function scopeNotCampaignAdvertisers($query) {
        return $query->whereDoesntHave('campaigns');
    }
