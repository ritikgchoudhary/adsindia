<template>
  <div id="faq" class="faq-section my-120">
    <div class="container">
          ...
          <div v-for="(chunk, chunkIndex) in faqChunks" :key="chunkIndex" class="col-lg-6">
            <div v-for="(faq, index) in chunk" :key="index" class="accordion-item">
              <h2 class="accordion-header" :id="'heading' + chunkIndex + index">
                ...
                >
                  {{ faq?.data_values?.question ?? faq?.question ?? '' }}
                </button>
              </h2>
              <div
              ...
              >
                <div class="accordion-body">
                  <p class="text">
                    {{ faq?.data_values?.answer ?? faq?.answer ?? '' }}
                  </p>
                </div>
              </div>
            </div>
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        ...
        faqContent.value = appService.getSectionContent(contentRes)
