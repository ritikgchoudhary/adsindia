      ...
      <div class="cta-wrapper">
        <div class="cta-content">
          ...
          <div class="cta-content__btn">
            <a v-if="ctaData?.button_url && !String(ctaData.button_url).startsWith('/')" :href="ctaData.button_url" class="btn btn--base pill">
              {{ ctaData?.button_name ?? '' }}
              <span class="btn-icon"><i class="las la-arrow-right"></i></span>
            </a>
            <router-link v-else :to="ctaData?.button_url || '#'" class="btn btn--base pill">
              {{ ctaData?.button_name ?? '' }}
              <span class="btn-icon"><i class="las la-arrow-right"></i></span>
            </router-link>
          </div>
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        ...
        ctaData.value = appService.getSectionContent(response)
      } catch (error) {
