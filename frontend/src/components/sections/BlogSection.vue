<template>
  <section id="blog" class="blog my-120">
    <div class="container">
          ...
          <div class="blog-item" :class="getBlogClass(index)">
            <div class="blog-item__thumb">
              <router-link :to="`/blog/${blog?.slug ?? ''}`" class="blog-item__thumb-link">
                <img :src="$getImage('blog', 'thumb_' + (blog?.data_values?.image ?? blog?.image ?? ''))" class="fit-image" alt="img">
              </router-link>
            ...
            <div class="blog-item__content">
              <span class="blog-item__badge">{{ blog?.data_values?.badge ?? blog?.badge ?? '' }}</span>
              <h5 class="blog-item__title">
                <router-link :to="`/blog/${blog?.slug ?? ''}`" class="blog-item__title-link">
                  {{ blog?.data_values?.title ?? blog?.title ?? '' }}
                </router-link>
              </h5>
            </div>
          </div>
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        ...
        blogContent.value = appService.getSectionContent(contentRes)
        const raw = elementsRes.data || []
        blogPosts.value = Array.isArray(raw) ? raw.slice(0, 3) : []
      } catch (error) {
